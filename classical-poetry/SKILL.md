...
本 skill 提供脚本工具，所有脚本调用遵循标准模式：

### 获取 Skill 路径

首次调用 skill 时，系统自动提供路径：
```json
// read_skill 输出示例
{
  "skill_id": "classical-poetry",
  "location": "~/.claude/skills/classical-poetry",
  "content": "...",
  "available_files": [...]
}
```
...
**使用 bash 工具 + working_dir 参数：**

```json
{
  "cmd": "python3 scripts/poetry_checker.py --mode ci --yun-shu 1 --poem '...'",
  "working_dir": "~/.claude/skills/classical-poetry"
}
```

**关键点：**
- ✅ **设置 working_dir**：使用从 `location` 字段获取的路径
- ✅ **相对路径**：脚本路径相对于 skill 目录（`scripts/xxx.py`）
- ✅ **会话缓存**：获取 location 后，整个会话中复用该值
- ❌ **不使用绝对路径**：避免硬编码 `python3 /absolute/path/xxx.py`
- ❌ **不询问用户**：路径由系统自动提供

### Python 代码示例

在 Python 代码中导入 skill 模块：

```python
# 假设 working_dir 已设置为 skill 目录
from scripts.poetry_checker import check_poem
from scripts.souyun_api import search_rhyme

# 或者使用相对路径
import sys
sys.path.insert(0, 'scripts')
from poetry_checker import check_poem
```

**占位符说明：**
- 下文中 `{location}` 仅用于说明，表示从 `read_skill` 获取的路径值
- 实际执行时，使用 `working_dir` 参数而不是在命令中拼接路径
...
```bash
python3 scripts/reference_builder.py \
  --keyword "[主题]" --pages 2 --scope Sentence --top 30
```

（使用 bash 工具时，设置 `working_dir` 为 skill 的 `location`）

**注意：** 脚本会直接输出 JSON 格式的结果，无需创建临时文件。
...
**对于诗：**
```bash
python3 scripts/poetry_checker.py \
